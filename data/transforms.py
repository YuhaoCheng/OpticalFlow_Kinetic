...
class ImageAugmentor:
    ...
    def spatial_transform(self, img1, img2, occlusion=None):
        # randomly sample scale
        ht, wd = img1.shape[:2]
        ...
        if occlusion is not None:
            occlusion = occlusion[y0:y0 + self.crop_size[0], x0:x0 + self.crop_size[1]]
            return img1, img2, occlusion

        return img1, img2

    def __call__(self, img1, img2, occlusion=None):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)

        if occlusion is not None:
            img1, img2, occlusion = self.spatial_transform(
                img1, img2, occlusion)
        else:
